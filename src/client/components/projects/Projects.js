...
const Projects = ({ openPane, togglePane, handleFilterChange, theme }) => {
    ...
    <section id="toSideProjects" className={`section is-projects ${theme}`}>
      <div className="content-wrap is-side-by-side">
        <div className="right-side">
          <div className="title-group has-filters">
            ...
            <p className="subtitle font-large">
              Getting my hands dirty with technologies I don't have a chance to work with in my day
              job helps me stay up to date.
            </p>
          </div>
          <fieldset className={`filter-section ${openPane === "projects" ? "active" : ""}`}>
            <button className="toggle-filter outline-button" onClick={() => togglePane("projects")}>
              Filters
            </button>
            ...
            <div className="filters">
              {filterItems.map((item) => (
                <label
                  key={item.tech}
                  className={`tag ${filterTerms.includes(item.tech) ? "active" : ""}`}
                >
                  <input
                    type="checkbox"
                    name={item.tech}
                    onChange={() => handleFilterChange(item.tech, filterTerms, setFilterTerms)}
                  />
        ...
        <div className="cards left-side">
          {projects
            .filter((project) => {
              return filterTerms.length ? isIncludes(project.stack, filterTerms) : true;
            })
            .map((project) => (
              <div
                key={project.id}
